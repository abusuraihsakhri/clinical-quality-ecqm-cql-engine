...
SUPPORTED_MEASURE_IDS = ("CMS130v11", "CMS122v11", "CMS125v11", "CMS165v11", "CMS68v12")
...
def format_measure_report(score: PopulationMeasureScore) -> str:
    ...
    lines.append(f" Improvement Notation: {direction} ({score.improvement_notation.value.upper()})")
    lines.append("=" * 78)
        ...
        for row in reader:
            ...
            if enc_code:
                pt.encounters.append(EncounterRecord(
                    ...
                    code=enc_code,
                    code_system="CPT",
                    period_start=row.get("encounter_date") or "2026-06-01"
                ))
            # Conditions
            ...
            if cond_code:
                pt.conditions.append(ConditionRecord(
                    ...
                    onset_date=row.get("condition_date") or "2020-01-01",
                    clinical_status=row.get("condition_status") or "active"
                ))
            # Observations
            ...
            if obs_code and obs_val is not None and obs_val != "":
                ...
                pt.observations.append(ObservationRecord(
                    ...
                    code_system="LOINC",
                    value=value,
                    date=row.get("observation_date") or "2026-06-01"
                ))
            # Procedures
            ...
            if proc_code:
                pt.procedures.append(ProcedureRecord(
                    code=proc_code,
                    code_system="CPT",
                    performed_date=row.get("procedure_date") or "2026-06-01"
                ))
            # Medications
            ...
            if med_code:
                pt.medications.append(MedicationRecord(
                    code=med_code,
                    code_system="RxNorm",
                    authored_date=row.get("medication_date") or "2026-06-01"
                ))
...
def main():
    ...
    # Batch subcommand
    p_batch = subparsers.add_parser("batch", help="Batch evaluate patient records from CSV")
    p_batch.add_argument("-i", "--input", required=True, help="Path to input CSV patient data")
    p_batch.add_argument("-o", "--output", default="results.csv", help="Path to output CSV results file")
    p_batch.add_argument("--measure", default="CMS130v11", choices=SUPPORTED_MEASURE_IDS, help="Target supported measure")

    # Root flags
    parser.add_argument("--demo", action="store_true", help="Run quality measures on a multi-patient sample cohort")
    parser.add_argument("--measure", type=str, default="CMS130v11", choices=SUPPORTED_MEASURE_IDS, help="Target supported measure")
